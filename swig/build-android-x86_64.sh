...
export ANDROID_TOOLCHAIN=/src/android-toolchain-x86_64
...
export run_bjam="${BOOST_ROOT}/b2 -j2 --user-config=config/${os_build}-${os_arch}-config.jam variant=release toolset=clang-${os_arch} target-os=${os_build} location=bin/release/${os_build}/${os_arch}"
...
#export CXX=${ANDROID_TOOLCHAIN}/bin/x86_64-linux-android${android_api}-clang++
