...
public class ChangeTrackersTest {
    ...
    public void testChangeTrackersUsingCreateTorrent() throws IOException {
        ...
        assertTrue(ti.trackers().length > 0);
        ...
        assertEquals(ti.trackers().length, 0);
        ...
        e = c.generate();
        ti = TorrentInfo.bdecode(Vectors.byte_vector2bytes(e.bencode()));
        AnnounceEntry[] trackers = ti.trackers();
        // do we have exactly the two added trackers
        assertEquals(trackers.length, 2);
        assertEquals(trackers[0].url(), "http://a:6969/announce");
        assertEquals(trackers[1].url(), "http://b:6969/announce");
    }
