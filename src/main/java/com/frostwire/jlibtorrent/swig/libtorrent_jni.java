...
public class libtorrent_jni {
    ...
    static {
        try {
            ...
            if ("".equals(path)) {
                try {
                    System.out.println("Trying jlibtorrent-" + jlibtorrentVersion() + ".<so|dylib>...");										
                    System.loadLibrary("jlibtorrent-" + jlibtorrentVersion());
										System.out.println("SUCCESS: Loaded jlibtorrent-" + jlibtorrentVersion());
                } catch (LinkageError e) {
                    // give it a try to the name without version
                    System.out.println("Failed loading jlibtorrent-" + jlibtorrentVersion() + ".<so|dylib>...");
                    e.printStackTrace();
                    try {
                        System.out.println("\n\nTrying jlibtorrent.<so|dylib>...");
                        System.loadLibrary("jlibtorrent");
                        System.out.println("SUCCESS: Loaded jlibtorrent.<so|dylib> (version=" +  jlibtorrentVersion() + ")");
                    } catch (Throwable t) {
												System.out.println("Failed loading jlibtorrent<so|dylib>");
							          t.printStackTrace();
										}
                }
            } else {
